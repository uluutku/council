...
Council uses encrypted connections, platform encryption for stored data at rest, private storage,
authentication, and Row Level Security. Trusted infrastructure processes content when needed for
delivery, search, attachments, and requested AI features, so Council does not claim end-to-end
encryption.

Application access is limited to authorized users and conversation members. The database remains
...
Authenticated users may read a conversation, its pair and membership data, messages, and reactions
only while their `auth.uid()` is a stored member. RLS-backed table grants are read-only.
Conversation creation, message writes, edits, tombstones, reactions, and receipt updates are
available only through fixed-search-path security-definer functions.
...
expires after seven days, and generation is denied when exhausted or expired. Only the
service-role `admin_set_ai_credits` hook may change balances or Pro status outside the normal
reservation, refund, and access-code redemption paths.
...
The provider prompt is assembled entirely on the server. The browser sends only the conversation
id, client id, and message content. Raw system instructions never reach the browser. Council's
platform safety preamble is always prepended and cannot be replaced or overridden by built-in
prompts or custom instructions. Personas may shape style but cannot grant themselves access to
human conversations, other users, files, credentials, hidden prompts, nonexistent tools, or the
internet, and the UI never claims such capabilities. Archived personas keep their history readable
but cannot start new generations until restored.
...
validates MIME signatures, computes SHA-256, and sends base64 bytes instead of a signed URL to the
...
### Persistent AI artifacts (Task 016)

`ai_artifacts` and `ai_artifact_versions` are owner-only under RLS. Browser roles can read their own
rows but cannot directly insert, update, or delete them. Narrow RPCs derive identity from
`auth.uid()`, enforce conversation and AI-contact ownership, validate source assistant messages,
limit active artifacts and content size, and make creation and version saves idempotent. Immutable
version rows reject direct updates and deletes; restore appends a new version.

Artifact revisions use the existing server-owned AI runtime and one normal credit reservation.
The server loads the saved artifact and contact identity rather than accepting a hidden prompt or
arbitrary artifact content from the browser. Artifact text is delimited as untrusted input below
fixed platform instructions. An archived persona cannot revise, and a streamed proposal cannot
overwrite the artifact until the owner explicitly saves it through a controlled RPC.

Artifacts are not shared with human contacts, unrelated AI conversations, or other users. Artifact
content is excluded from logs and Realtime events, and exports contain only the current saved plain
text version with a sanitized local filename.

### Messaging polish and Premium codes (Task 017)

Only AI assistant output is interpreted as Markdown. Raw HTML is not enabled, unsafe protocols are
discarded, remote Markdown images never load, and code copy uses text APIs rather than HTML
injection. Streaming partial Markdown is presentation-only and does not alter strict SSE
termination.

Typing events contain only version, event name, and timestamp. They are private, ephemeral,
member-authorized, unlogged, and cannot be sent to durable topics. Presence exposes no activity
history: bounded RPCs return only policy-filtered online and last-seen values to accepted,
unblocked contacts. Mute preferences are owner-only and suppress browser notification and sound,
not unread counts.

Premium plaintext codes exist only in owner CLI memory/output at creation and user input at
redemption. PostgreSQL stores SHA-256 hashes and short prefixes; browser roles cannot read the code
table or modify AI balances. Invalid redemption results are generic and limited to ten failed
attempts per user per hour. Run-level `credit_source` ensures failures and stale recovery refund
the same Premium or trial pool exactly once.
