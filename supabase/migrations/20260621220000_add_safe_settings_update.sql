...
begin
  if p_theme is null
    ...
    and p_privacy_preferences is null then
...
comment on function public.update_my_settings(text, jsonb, jsonb) is
  'Merges supported authenticated-user settings fields while preserving unrelated stored keys.';

revoke all on function public.update_my_settings(text, jsonb, jsonb)
  from public, anon, authenticated;

grant execute on function public.update_my_settings(text, jsonb, jsonb)
  to authenticated;
