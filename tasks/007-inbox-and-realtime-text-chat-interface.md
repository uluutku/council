...
reactions, honest delivery/read display, unread counts, realtime synchronization with gap and
...
message shows an honest single indicator — Sent, then Delivered, then Read — never a fabricated
per-message status. Advancing read patches the inbox cache so the unread badge clears without a
refetch.
...
without blocking, subscribe — returning the handle immediately). The previous `await setAuth()`
